...
function connectToTable($user, $password, $databaseName, $tableName) {
	...
	if($isExistsTable == false) {
		$result = $mysqlConnect->query("create table ".$tableName." (id INT(10) AUTO_INCREMENT PRIMARY KEY, event_id INT(11), ticket_type VARCHAR(20), event_date DATETIME, ticket_adult_price INT(11), ticket_adult_quantity INT(11), ticket_kid_price INT(11), ticket_kid_quantity INT(11), barcode VARCHAR(120), equal_price INT(11), created DATETIME);");
		if($result == false)
			echo($mysqlConnect->error."\n");
	}
...
function insertDataIntoTable($mysqlConnect, $tableName, $event_id, $ticket_type, $event_date, $ticket_adult_price, $ticket_adult_quantity, $ticket_kid_price, $ticket_kid_quantity) {
	...
	do {
		...
		$barcode = hash("md5", $event_id.$event_date.$equal_price.$ticket_type.$created, $binary = false);
		...
		if($result) {
			...
			$query = "insert into ".$tableName." (event_id, event_date, ticket_type, ticket_adult_price, ticket_adult_quantity, ticket_kid_price, ticket_kid_quantity, barcode, equal_price, created) values (".$event_id.", \"".$event_date."\", \"".$ticket_type."\", ".$ticket_adult_price.", ".$ticket_adult_quantity.", ".$ticket_kid_price.", ".$ticket_kid_quantity.", \"".$barcode."\", ".$equal_price.", \"".$created."\");";
			echo($query);
